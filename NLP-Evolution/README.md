...
## RAG без эмбеддингов
### Вызовы перед векторной базой данных
* Высокий `similarity` для кейсов, которые нам не подходят
* Эмбеддинги очень сильно цепляются к словам и формулировкам и дают высокий скор
* В пользовательских запросах часто присутствует `бинарная логика` - хочу выполнять и этот критерий, и этот критерий
* Общение в чате или телеграм канале контекстно зависимо, поэтому хорошо работает нарезание чанков по абзацу
### Устранение симптомов
* Би-энкодеры и кросс-энкодеры
* Отдельный промпт для обработки `query`, переформулировать запрос под язык, принятый в доменной области
* Обработать и чанки базы данных
* [Instruction Awareness](https://ai.google.dev/gemini-api/docs/embeddings?hl=ru#supported-task-types)
* Если оба поднимаемых предложения базы знаний схожи семантически, то вычленить запрашиваемую сущность `детерминированными` правилами
### Альтернативный подход
* Берём не абзац, а страницу
* Предобработка страниц - Вопрос а есть ли информация по ответу - Бинарный ответ - Generattive
* Вместо вопроса можно Checklist, Checkbox, Custom Reasoning
* Номер страницы получить сразу, а потом по ним уже подтягивать информацию на Generative
* Теги, тегирование информации
* Text-to-SQL если анализ отчётности - детерминированный алгоритм, который умеет доставать структурированные данные
* Routing если контекста много
* Google модель хорошо вытаскивает графики и картинки из PDF
