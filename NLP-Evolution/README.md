...
## RAG без эмбеддингов
### Вызовы, которые возникают перед векторной базой данных
`Высокий similarity для кейса, который нам не подходит`
1. Устранить симпотомы
* Отдельный промпт для обработки query
* Если оба предложения похожи семантически, то вычленить дату детерминированными правилами
